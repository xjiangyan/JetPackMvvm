...
//            } catch (e: Exception) {
//                Log.e("debug", "viewModelScope.Exception: ${e.localizedMessage}")
//                when (e) {
...
//        }
        ...
        viewModelScope.launch(Dispatchers.Main + CoroutineExceptionHandler { _, throwable ->
            Log.e("debug", "viewModelScope.Exception: ${throwable.localizedMessage}")
            mStateLiveData.value = ErrorState(throwable.localizedMessage)
        }) {
            mStateLiveData.value = LoadState
            withContext(Dispatchers.IO) {
                block()
